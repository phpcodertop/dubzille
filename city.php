...
$dedc=mysql_query(" select * from  city  where ID=$city_id  limit 0,1");
while($result=mysql_fetch_array($dedc))
{
?>
<?=$result['country_address']?>
|
<?=$result['address']?>
<?php
}
...
وجدنا
<?
$deecount=mysql_query("select COUNT(ID) as ads_count FROM  ads where city_id='$city_id'  and  status='موافقة' ");
...
اعلان
	...
	*/
	$city_id=$_GET['city_id'];
	$query = "SELECT COUNT(*) as num FROM ads where city_id=$city_id  and  status='موافقة' ";
	$total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $total_pages[num];
	
	/* Setup vars for query. */
	$targetpage = "city.php?city_id=$city_id"; 	//your file name  (the name of this file)
	$limit = 48; 								//how many items to show per page  
	$page = $_GET['page'];
	if($page) 
	...
	$sql = "SELECT * FROM ads where city_id=$city_id  and  status='موافقة' order by ID desc LIMIT $start,$limit ";
            ...
            <div class="d-listing__item">
                    ...
                    <div class="u-c u-c--5o12 u-c--t3o12 u-c--d2o12 u-helper--no-padding">
                        <div class="u-r d-listing__photo">
                            ...
                            <img width="100%" class="u-img__img" src="admin/<?=$row['ImagePath']?>">
                            </a>
                        </div>
                    </div>
