...
|
<?php
$catb_id=$_GET['catb_id'];
$dedc=mysql_query(" select * from  ads_catb  where ID=$catb_id  limit 0,1");
while($result=mysql_fetch_array($dedc))
{
$cata_id=$result['cat_id'] ;
?>		
<?=$result['cat_name']?>
|
<?=$result['address']?>		
...
<?
include_once("supcategory_right.php");
?> 
...
?>	    
            <li>
                <a class="u-link" href="category.php?cat_id=<?=$cata_id?>">                    
                <?=$result['cat_name']?>                  
                </a>
            </li>	    
<?php
...
<?
$catb_id=$_GET['catb_id'];
$deecount=mysql_query("select COUNT(ID) as ads_count FROM  ads where catb_id='$catb_id' and  country_id='$country_id' and  status='موافقة'  ");
...
اعلان
	...
	*/
	$catb_id=$_GET['catb_id'];
	$query = "SELECT COUNT(*) as num FROM ads where catb_id=$catb_id and  country_id='$country_id' and  status='موافقة' ";
	$total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $total_pages[num];
	
	/* Setup vars for query. */
	$targetpage = "supcategory.php?catb_id=$catb_id"; 	//your file name  (the name of this file)
	...
	$sql = "SELECT * FROM ads where catb_id=$catb_id and  country_id='$country_id' and  status='موافقة' order by ID desc LIMIT $start,$limit ";
