...
include_once("../admin/connection.php");
		$deeteatcher=mysql_query("select * from   country where ID='$country_id' ");
	        $resteatcher=mysql_fetch_array($deeteatcher);
	        $country_id=$resteatcher['ID'];
		$country_address=$resteatcher['address'];
		echo $country_address ;
?>	
...
switch($_REQUEST['txtAction'])
{
	case "1":
          ...
          mysql_query(" insert into city  (address,country_id,country_address,appearance) values ('$_POST[txtaddress]','$country_id','$country_address','$_POST[ddlappearance]')");
		  
           if($page=='')
             {		   
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_city.php?country_id=$country_id' ";
           echo " </script>";
		     }
			 else
			 {
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_city.php?country_id=$country_id&page=$page' ";
           echo " </script>";
			 }
	...
	case "3":
	   $ID=$_POST['txtId'];
	   $desEdit=mysql_query("select * from city where ID=$ID");
	 ...
	 case "2":
	      ...
	      $ID=$_POST['txtId'];
			...
			mysql_query("update city set address='$_POST[txtaddress]',appearance='$_POST[ddlappearance]' where ID=$ID");
...
<script language='javascript' type='text/javascript'>
    function btnsave_Click()
	{
		 if(document.getElementById('txtId').value=='-1')
		 {
			 ...
			 {
			    alert('من فضلك اكتب عنوان المدينة.');
			   return false;
			 }
...
<h3 class="hndle"><span>مدن الدولة
&nbsp;&nbsp;&nbsp;
<?php
		  $deeteatcher=mysql_query("select * from   country where ID='$country_id' ");
	      $resteatcher=mysql_fetch_array($deeteatcher);
...
<div style="" class="inside">
	...
	<form id="frmcategory" action="" method="POST" enctype="multipart/form-data">
      ...
      <input type='hidden' id='txtAction' name='txtAction' value=''>
		...
		<div >
			<input name="txtaddress" id="txtaddress" tabindex="1" size='185' type="text" value="<?=$address;?>">
		</div>
...
$cat_id=$_GET['cat_id'];
	...
	if(!$recordsPerPage){
		...
		$i=($page-1)*$recordsPerPage;
		 ...
		 $date= @date("Y-m-d") ;
		$countRes=mysql_query("select count(ID)from city where country_id='$country_id' ");
		 ...
		 $date= @date("Y-m-d") ;
		if(!$result=mysql_query("select * from city  where country_id='$country_id'  order by ID desc  limit $i,$recordsPerPage  ")){
		...
		if($page>1){
		echo "<td>";		
		echo "<a class='zo' href= admin_city.php?country_id=$country_id>الاولى</a>";
		echo "</td>";
        echo "<td>";		
		echo "<a class='zo' href= admin_city.php?country_id=$country_id&page=".($page-1).">السابق</a>";
		echo "</td>";
	}else{
	...
	//here also i must change the path to the same page that i work in;
	    echo "<td>";
		echo "<a class='zo' href= admin_city.php?country_id=$country_id&page=".($page+1).">التالى</a>";
		echo "</td>";
		echo "<td>";
		echo "<a class='zo' href= admin_city.php?country_id=$country_id&page=".$totalPages.">الاخيره</a>";
		echo "</td>";
	}else{
...
<table class="wp-list-table widefat fixed posts" cellspacing="0">
	...
	<tr>
        ...
        <th style='width:60%;' scope="col" id="categories" class="manage-column column-categories" style="">المدينة</th>
