...
switch($_REQUEST['txtAction'])
{
	case "1":
          ...
          mysql_query(" insert into  newpage  (address,details) values ('$_POST[txtaddress]','$_POST[txtdetails]')");
		  
           if($page=='')
             {		   
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_newpage.php' ";
           echo " </script>";
		     }
			 else
			 {
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_newpage.php?page=$page' ";
           echo " </script>";
			 }
	...
	case "3":
	   $ID=$_POST['txtId'];
	   $desEdit=mysql_query("select * from newpage where ID=$ID");
	 ...
	 case "2":
	      ...
	      $ID=$_POST['txtId'];
			...
			mysql_query("update newpage set address='$_POST[txtaddress]',details='$_POST[txtdetails]' where ID=$ID");
	 ...
	 case "4":
	   ...
	   mysql_query("Delete from newpage where ID=$ID");
...
<h3 class="hndle"><span>اضافة صفحة جديده	  
</span></h3>
...
$cat_id=$_GET['cat_id'];
	...
	if(!$recordsPerPage){
		...
		$i=($page-1)*$recordsPerPage;
		 ...
		 $date= @date("Y-m-d") ;
		$countRes=mysql_query("select count(ID)from  newpage     ");
		 ...
		 $date= @date("Y-m-d") ;
		if(!$result=mysql_query("select * from  newpage    order by ID desc  limit $i,$recordsPerPage  ")){
		...
		if($page>1){
		echo "<td>";		
		echo "<a class='zo' href= admin_newpage.php>الاولى</a>";
		echo "</td>";
        echo "<td>";		
		echo "<a class='zo' href= admin_newpage.php?page=".($page-1).">السابق</a>";
		echo "</td>";
	}else{
	...
	//here also i must change the path to the same page that i work in;
	    echo "<td>";
		echo "<a class='zo' href= admin_newpage.php?page=".($page+1).">التالى</a>";
		echo "</td>";
		echo "<td>";
		echo "<a class='zo' href= admin_newpage.php?page=".$totalPages.">الاخيره</a>";
		echo "</td>";
	}else{
...
<table class="wp-list-table widefat fixed posts" cellspacing="0">
	...
	<tr>
        ...
        <th style='width:70%;' scope="col" id="categories" class="manage-column column-categories" style="">العنوان</th>
...
<?php
	...
	<a href="../newpage.php?page_id=<?=$record['0'];?>" target="_blank">شاهد الصفحة</a>
	</td>
	</tr>
