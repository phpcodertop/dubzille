...
switch($_REQUEST['txtAction'])
{
	case "1":
	      $page=$_GET['page'];
          mysql_query(" insert into country  (address,appearance) values ('$_POST[txtaddress]','$_POST[ddlappearance]')");

		  
           if($page=='')
             {		   
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_country.php' ";
           echo " </script>";
		     }
			 else
			 {
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_country.php?page=$page' ";
           echo " </script>";
			 }
	...
	case "3":
	   $ID=$_POST['txtId'];
	   $desEdit=mysql_query("select * from country where ID=$ID");
	 ...
	 case "2":
	    ...
	    mysql_query("update city set country_address='$_POST[txtaddress]' where country_id='$ID' ");
...
<h3 class="hndle"><span>الدول</span></h3>
...
$page=$_GET['page'];
	...
	if(!$recordsPerPage){
		...
		$i=($page-1)*$recordsPerPage;
		 ...
		 $date= @date("Y-m-d") ;
		$countRes=mysql_query("select count(ID)from  country  ");
		 ...
		 $date= @date("Y-m-d") ;
		if(!$result=mysql_query("select * from  country   order by ID desc  limit $i,$recordsPerPage  ")){
		...
		if($page>1){
		echo "<td>";		
		echo "<a class='zo' href= admin_country.php>الاولى</a>";
		echo "</td>";
        echo "<td>";		
		echo "<a class='zo' href= admin_country.php?page=".($page-1).">السابق</a>";
		echo "</td>";
	}else{
	...
	//here also i must change the path to the same page that i work in;
	    echo "<td>";
		echo "<a class='zo' href= admin_country.php?page=".($page+1).">التالى</a>";
		echo "</td>";
		echo "<td>";
		echo "<a class='zo' href= admin_country.php?page=".$totalPages.">الاخيره</a>";
		echo "</td>";
	}else{
...
<table class="wp-list-table widefat fixed posts" cellspacing="0">
	...
	<tr>
        ...
        <th style='width:50%;' scope="col" id="categories" class="manage-column column-categories" style="">العنوان</th>
	<th style='width:20%;' scope="col" id="categories" class="manage-column column-categories" style="">ترتيب الظهور</th>
	<th style='width:15%;' scope="col" id="tags" class="manage-column column-tags" style="">حركات</th>
	</tr>
	</thead>
...
<?php
	...
	&nbsp;&nbsp;&nbsp;
	<a href='admin_city.php?country_id=<?=$record['0'];?>'> المدن </a>
