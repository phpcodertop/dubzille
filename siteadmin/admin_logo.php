...
switch($_REQUEST['txtAction'])
{
	...
	case "3":
	   $ID=$_POST['txtId'];
	   $desEdit=mysql_query("select * from logo where ID=$ID");
	 ...
	 case "2":
		  ...
		  $SelectImagePath=mysql_query("Select ImagePath from logo  where ID=$ID");
		  ...
		  if ($_FILES['UploadlogoFile']['name'] != null && $_FILES['UploadlogoFile']['name'] != '')
	 {
		  ...
		  move_uploaded_file($_FILES['UploadlogoFile']['tmp_name'],$Trget_ImagePath_upload);
			...
			mysql_query("update logo set address='$_POST[txtaddress]',ImagePath='".$Trget_ImagePath."' where ID=$ID");
			
	 }
		  else		  
		  {
			mysql_query("update logo set address='$_POST[txtaddress]' where ID=$ID");
		  }
           ...
           echo "  window.location = 'admin_logo.php' ";
           echo " </script>";
		     }
			 else
			 {
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_logo.php?page=$page' ";
           echo " </script>";
			 }
	 break;
...
<h3 class="hndle"><span>شعار الموقع</span></h3>
...
$page=$_GET['page'];
	...
	if(!$recordsPerPage){
		...
		$i=($page-1)*$recordsPerPage;
		 ...
		 $date= @date("Y-m-d") ;
		$countRes=mysql_query("select count(ID)from  logo  ");
		 ...
		 $date= @date("Y-m-d") ;
		if(!$result=mysql_query("select * from  logo   order by ID desc  limit $i,$recordsPerPage  ")){
		...
		if($page>1){
		echo "<td>";		
		echo "<a class='zo' href= admin_logo.php>الاولى</a>";
		echo "</td>";
        echo "<td>";		
		echo "<a class='zo' href= admin_logo.php?page=".($page-1).">السابق</a>";
		echo "</td>";
	}else{
	...
	//here also i must change the path to the same page that i work in;
	    echo "<td>";
		echo "<a class='zo' href= admin_logo.php?page=".($page+1).">التالى</a>";
		echo "</td>";
		echo "<td>";
		echo "<a class='zo' href= admin_logo.php?page=".$totalPages.">الاخيره</a>";
		echo "</td>";
	}else{
