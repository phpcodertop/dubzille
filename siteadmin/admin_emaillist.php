...
switch($_REQUEST['txtAction'])
{
	case "1":
          ...
          mysql_query(" insert into emiallist  (email) values ('$_POST[txtemail]')");
		  
           if($page=='')
             {		   
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_emaillist.php' ";
           echo " </script>";
		     }
			 else
			 {
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_emaillist.php?page=$page' ";
           echo " </script>";
			 }
	...
	case "3":
	   $ID=$_POST['txtId'];
	   $desEdit=mysql_query("select * from emiallist where ID=$ID");
	 ...
	 case "2":
	      ...
	      $ID=$_POST['txtId'];
			...
			mysql_query("update emiallist set email='$_POST[txtemail]' where ID=$ID");
	 ...
	 case "4":
	   ...
	   mysql_query("Delete from emiallist where ID=$ID");
...
<script language='javascript' type='text/javascript'>
    function btnsave_Click()
	{
		 if(document.getElementById('txtId').value=='-1')
		 {
			 ...
			 {
			    alert('من فضلك اكتب البريد الالكترونى.');
			   return false;
			 }
	 	 ...
	 	 function btnsearch_Click()
	{
		 ...
		 {  
			 ...
			 {
			 document.getElementById('txtAction').value="5";
             }  			 
		 }
...
<h3 class="hndle"><span>القائمة البريدية</span></h3>
<div style="" class="inside">
	...
	<form id="frmcategory" action="" method="POST" enctype="multipart/form-data">
      ...
      <input type='hidden' id='txtAction' name='txtAction' value=''>
		...
		<h4 id="quick-post-title"><label for="title">البريد الالكترونى</label></h4>
		<div >
		...
		<p class="submit">
				...
				<input name="btnsearch" id="btnsearch" class="button-primary" value="بحث" type="submit" onclick="return btnsearch_Click()" >
...
<?php
	...
	<thead>
	<tr>
        <th style='width:80%;' scope="col" id="categories" class="manage-column column-categories" style="">البريد الالكترونى</th>
...
<?php
	    $dessearch=mysql_query(" select * from emiallist   WHERE  email LIKE ('$_POST[txtemail]')
             order by ID desc  ");
         while($result=mysql_fetch_array($dessearch))
		 {
         ?>
	<tr id="post-1175" class="author-self status-publish format-default iedit" valign="top">
							
	<td class="hsm_pagetitle column-hsm_pagetitle"><?=$result['email']?></td>
	<td class="hsm_pagetitle column-hsm_pagetitle">
	<input type='button' id='btnEdit' value='تعديل' onclick='btnEdit_Click(<?=$result['ID'];?>)'>
	<input type='button' id='btndelete' value='حذف' onclick='btndelete_Click(<?=$result['ID'];?>)'>
	</td>
	</tr>
...
$page=$_GET['page'];
	...
	if(!$recordsPerPage){
		...
		$i=($page-1)*$recordsPerPage;
		 ...
		 $date= @date("Y-m-d") ;
		$countRes=mysql_query("select count(ID)from emiallist  ");
		 ...
		 $date= @date("Y-m-d") ;
		if(!$result=mysql_query("select * from emiallist   order by ID desc  limit $i,$recordsPerPage  ")){
		...
		if($page>1){
		echo "<td>";		
		echo "<a class='zo' href= admin_emaillist.php>الاولى</a>";
		echo "</td>";
        echo "<td>";		
		echo "<a class='zo' href= admin_emaillist.php?page=".($page-1).">السابق</a>";
		echo "</td>";
	}else{
	...
	//here also i must change the path to the same page that i work in;
	    echo "<td>";
		echo "<a class='zo' href= admin_emaillist.php?page=".($page+1).">التالى</a>";
		echo "</td>";
		echo "<td>";
		echo "<a class='zo' href= admin_emaillist.php?page=".$totalPages.">الاخيره</a>";
		echo "</td>";
	}else{
