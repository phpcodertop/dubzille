...
switch($_REQUEST['txtAction'])
{
	case "1":
	      $page=$_GET['page'];
          mysql_query(" insert into currency  (address,appearance) values ('$_POST[txtaddress]','$_POST[ddlappearance]')");
		  
           if($page=='')
             {		   
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_currency.php' ";
           echo " </script>";
		     }
			 else
			 {
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_currency.php?page=$page' ";
           echo " </script>";
			 }
	...
	case "3":
	   $ID=$_POST['txtId'];
	   $desEdit=mysql_query("select * from currency where ID=$ID");
	 ...
	 case "2":
	      ...
	      $ID=$_POST['txtId'];
			...
			mysql_query("update currency set address='$_POST[txtaddress]',appearance='$_POST[ddlappearance]' where ID=$ID");
	 ...
	 case "4":
	   ...
	   mysql_query("Delete from currency where ID=$ID");
...
<h3 class="hndle"><span>العملات</span></h3>
...
$page=$_GET['page'];
	...
	if(!$recordsPerPage){
		...
		$i=($page-1)*$recordsPerPage;
		 ...
		 $date= @date("Y-m-d") ;
		$countRes=mysql_query("select count(ID)from  currency  ");
		 ...
		 $date= @date("Y-m-d") ;
		if(!$result=mysql_query("select * from  currency   order by ID desc  limit $i,$recordsPerPage  ")){
		...
		if($page>1){
		echo "<td>";		
		echo "<a class='zo' href= admin_currency.php>الاولى</a>";
		echo "</td>";
        echo "<td>";		
		echo "<a class='zo' href= admin_currency.php?page=".($page-1).">السابق</a>";
		echo "</td>";
	}else{
	...
	//here also i must change the path to the same page that i work in;
	    echo "<td>";
		echo "<a class='zo' href= admin_currency.php?page=".($page+1).">التالى</a>";
		echo "</td>";
		echo "<td>";
		echo "<a class='zo' href= admin_currency.php?page=".$totalPages.">الاخيره</a>";
		echo "</td>";
	}else{
