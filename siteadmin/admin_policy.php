...
switch($_REQUEST['txtAction'])
{
	...
	case "3":
	   $ID=$_POST['txtId'];
	   $desEdit=mysql_query("select * from policy where ID=$ID");
	 ...
	 case "2":
	      ...
	      $ID=$_POST['txtId'];
			...
			mysql_query("update policy set details='$_POST[txtdetails]' where ID=$ID");
           ...
           echo "  window.location = 'admin_policy.php' ";
           echo " </script>";
		     }
			 else
			 {
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_policy.php?page=$page' ";
           echo " </script>";
			 }
	 break;
	 case "4":
	   ...
	   mysql_query("Delete from policy where ID=$ID");
...
<h3 class="hndle"><span>سياسة الخصوصية</span></h3>
...
$page=$_GET['page'];
	...
	if(!$recordsPerPage){
		...
		$i=($page-1)*$recordsPerPage;
		 ...
		 $date= @date("Y-m-d") ;
		$countRes=mysql_query("select count(ID)from policy  ");
		 ...
		 $date= @date("Y-m-d") ;
		if(!$result=mysql_query("select * from policy   order by ID desc  limit $i,$recordsPerPage  ")){
		...
		if($page>1){
		echo "<td>";		
		echo "<a class='zo' href= admin_policy.php>الاولى</a>";
		echo "</td>";
        echo "<td>";		
		echo "<a class='zo' href= admin_policy.php?page=".($page-1).">السابق</a>";
		echo "</td>";
	}else{
	...
	//here also i must change the path to the same page that i work in;
	    echo "<td>";
		echo "<a class='zo' href= admin_policy.php?page=".($page+1).">التالى</a>";
		echo "</td>";
		echo "<td>";
		echo "<a class='zo' href= admin_policy.php?page=".$totalPages.">الاخيره</a>";
		echo "</td>";
	}else{
...
<table class="wp-list-table widefat fixed posts" cellspacing="0">
	...
	<tr>
        ...
        <th style='width:80%;' scope="col" id="categories" class="manage-column column-categories" style="">سياسة الخصوصية</th>
