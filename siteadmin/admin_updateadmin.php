...
switch($_REQUEST['txtAction'])
{
	case "1":
	      $page=$_GET['page'];
          mysql_query(" insert into admin  (admin_name,username,password) values ('$_POST[txtadmin_name]','$_POST[txtadmin_username]','$_POST[txtpassword]')");
		  
           if($page=='')
             {		   
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_updateadmin.php' ";
           echo " </script>";
		     }
			 else
			 {
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_updateadmin.php?page=$page' ";
           echo " </script>";
			 }
	...
	case "3":
	   $ID=$_POST['txtId'];
	   $desEdit=mysql_query("select * from admin where ID=$ID");
	   $res=mysql_fetch_array($desEdit);
	   $admin_name_2=$res['admin_name'];
	 ...
	 case "2":
	      ...
	      $ID=$_POST['txtId'];
			...
			mysql_query("update admin set admin_name='$_POST[txtadmin_name]',username='$_POST[txtadmin_username]',password='$_POST[txtpassword]' where ID=$ID");
...
<script language='javascript' type='text/javascript'>
    function btnsave_Click()
	{
		 ...
		 {
			 if(document.getElementById('txtadmin_name').value=='')
			 {
			    alert('من فضلك اكتب اسم الادمين.');
			   return false;
			 }
...
<h3 class="hndle"><span>تعديل بيانات الدخول</span></h3>
<div style="" class="inside">
	...
	<form id="frmcategory" action="" method="POST" enctype="multipart/form-data">
      ...
      <input type='hidden' id='txtAction' name='txtAction' value=''>
		...
		<h4 id="quick-post-title"><label for="title">اسم الادمين &nbsp;&nbsp;&nbsp;</label></h4>
		<div >
			<input name="txtadmin_name" id="txtadmin_name" size='180'  type="text" value="<?=$admin_name_2;?>">
		</div>
		...
		<div >
			<input name="txtadmin_username" id="txtadmin_username" size='180'  type="text" value="<?=$admin_username;?>">
		</div>
		...
		<div  >
			<input name="txtpassword" id="txtpassword" size='180'  type="text" value="<?=$password;?>">
		</div>
...
$page=$_GET['page'];
	...
	if(!$recordsPerPage){
		...
		$i=($page-1)*$recordsPerPage;
		 ...
		 $date= @date("Y-m-d") ;
		$countRes=mysql_query("select count(ID)from admin  where ID=$User_ID ");
		 ...
		 $date= @date("Y-m-d") ;
		if(!$result=mysql_query("select * from admin  where ID=$User_ID  order by ID desc  limit $i,$recordsPerPage  ")){
		...
		if($page>1){
		echo "<td>";		
		echo "<a class='zo' href= admin_updateadmin.php>الاولى</a>";
		echo "</td>";
        echo "<td>";		
		echo "<a class='zo' href= admin_updateadmin.php?page=".($page-1).">السابق</a>";
		echo "</td>";
	}else{
	...
	//here also i must change the path to the same page that i work in;
	    echo "<td>";
		echo "<a class='zo' href= admin_updateadmin.php?page=".($page+1).">التالى</a>";
		echo "</td>";
		echo "<td>";
		echo "<a class='zo' href= admin_updateadmin.php?page=".$totalPages.">الاخيره</a>";
		echo "</td>";
	}else{
...
<table class="wp-list-table widefat fixed posts" cellspacing="0">
	<thead>
	<tr>
        <th style='width:80px;' scope="col" id="categories" class="manage-column column-categories" style="">الرقم</th>
        <th style='width:300px;' scope="col" id="categories" class="manage-column column-categories" style="">اسم المهندس</th>
		<th style='width:150px;' scope="col" id="categories" class="manage-column column-categories" style="">اسم المستخدم</th>
		<th style='width:150px;' scope="col" id="categories" class="manage-column column-categories" style="">كلمة السر</th>
