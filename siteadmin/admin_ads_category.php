...
switch($_REQUEST['txtAction'])
{
	case "1":
	      $page=$_GET['page'];
          mysql_query(" insert into ads_cat  (address,ImagePath,appearance) values ('$_POST[txtaddress]','-','$_POST[ddlappearance]')");
		  $MaxID=mysql_query("SELECT MAX(ID) ID FROM ads_cat ");
		  ...
		  mysql_query('Update ads_cat  set ImagePath="'.$Trget_ImagePath.'" Where ID='.$res['ID']);
		  
           if($page=='')
             {		   
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_ads_category.php' ";
           echo " </script>";
		     }
			 else
			 {
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_ads_category.php?page=$page' ";
           echo " </script>";
			 }
	...
	case "3":
	   $ID=$_POST['txtId'];
	   $desEdit=mysql_query("select * from ads_cat where ID=$ID");
	 ...
	 case "2":
	      $page=$_GET['page'];
	      $ID=$_POST['txtId'];
		  $SelectImagePath=mysql_query("Select ImagePath from ads_cat  where ID=$ID");
		  ...
		  if ($_FILES['UploadlogoFile']['name'] != null && $_FILES['UploadlogoFile']['name'] != '')
	 {
		  ...
		  move_uploaded_file($_FILES['UploadlogoFile']['tmp_name'],$Trget_ImagePath_upload);
			...
			mysql_query("update ads_cat set address='$_POST[txtaddress]',ImagePath='".$Trget_ImagePath."',appearance='$_POST[ddlappearance]' where ID=$ID");
			
	 }
		  else		  
		  {
			mysql_query("update ads_cat set address='$_POST[txtaddress]',appearance='$_POST[ddlappearance]' where ID=$ID");
		  }


	    $de_update=mysql_query(" select * from ads_catb where  cat_id='$ID' ");
		while($result_update=mysql_fetch_array($de_update))
		{
	    mysql_query("update ads_catb set country_address='$_POST[txtaddress]' where cat_id='$ID' ");
	    }	
	 ...
	 case "4":
	   ...
	   mysql_query("Delete from ads_cat where ID=$ID");
...
<h3 class="hndle"><span>اقسام الاعلانات</span></h3>
...
$page=$_GET['page'];
	...
	if(!$recordsPerPage){
		...
		$i=($page-1)*$recordsPerPage;
		 ...
		 $date= @date("Y-m-d") ;
		$countRes=mysql_query("select count(ID)from  ads_cat  ");
		 ...
		 $date= @date("Y-m-d") ;
		if(!$result=mysql_query("select * from  ads_cat   order by ID desc  limit $i,$recordsPerPage  ")){
		...
		if($page>1){
		echo "<td>";		
		echo "<a class='zo' href= admin_ads_category.php>الاولى</a>";
		echo "</td>";
        echo "<td>";		
		echo "<a class='zo' href= admin_ads_category.php?page=".($page-1).">السابق</a>";
		echo "</td>";
	}else{
	...
	//here also i must change the path to the same page that i work in;
	    echo "<td>";
		echo "<a class='zo' href= admin_ads_category.php?page=".($page+1).">التالى</a>";
		echo "</td>";
		echo "<td>";
		echo "<a class='zo' href= admin_ads_category.php?page=".$totalPages.">الاخيره</a>";
		echo "</td>";
	}else{
...
<table class="wp-list-table widefat fixed posts" cellspacing="0">
	...
	<tr>
        ...
        <th style='width:30%;' scope="col" id="categories" class="manage-column column-categories" style="">العنوان</th>
		<th style='width:20%;' scope="col" id="categories" class="manage-column column-categories" style="">ترتيب الظهور</th>
		<th style='width:20%;' scope="col" id="categories" class="manage-column column-categories" style="">الصورة</th>
...
<?php
	...
	<td class="hsm_pagetitle column-hsm_pagetitle"><img src="../<?=$ImagePath?>" width='28' height='28' /></td>
	...
	&nbsp;&nbsp;&nbsp;
	<a href='admin_ads_category_b.php?cat_id=<?=$record['0'];?>'> الاقسام الفرعية </a>
