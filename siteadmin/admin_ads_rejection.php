	...
	case "1":
		...
		if($_FILES['UploadlogoFile']['name'] == '')
		  ...
		  mysql_query('Update ads  set ImagePath="'.$Trget_ImagePath.'" Where ID='.$res['ID']);		  
           ...
           echo "  window.location = 'admin_ads_rejection.php' ";
           echo " </script>";
		     }
			 else
			 {
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_ads_rejection.php?page=$page' ";
           echo " </script>";
			 }
...
$cat_id=$_GET['cat_id'];
	...
	if(!$recordsPerPage){
		...
		$i=($page-1)*$recordsPerPage;
		 ...
		 $date= @date("Y-m-d") ;
		$countRes=mysql_query("select count(ID)from ads where status='رفض' ");
		 ...
		 $date= @date("Y-m-d") ;
		if(!$result=mysql_query("select * from  ads where status='رفض' order by ID desc  limit $i,$recordsPerPage  ")){
		...
		if($page>1){
		echo "<td>";		
		echo "<a class='zo' href= admin_ads_rejection.php>الاولى</a>";
		echo "</td>";
        echo "<td>";		
		echo "<a class='zo' href= admin_ads_rejection.php?page=".($page-1).">السابق</a>";
		echo "</td>";
	}else{
	...
	//here also i must change the path to the same page that i work in;
	    echo "<td>";
		echo "<a class='zo' href= admin_ads_rejection.php?page=".($page+1).">التالى</a>";
		echo "</td>";
		echo "<td>";
		echo "<a class='zo' href= admin_ads_rejection.php?page=".$totalPages.">الاخيره</a>";
		echo "</td>";
	}else{
