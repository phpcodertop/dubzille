...
<title>مواقع التواصل الاجتماعى</title>
...
switch($_REQUEST['txtAction'])
{
	...
	case "3":
	   $ID=$_POST['txtId'];
	   $desEdit=mysql_query("select * from otherlink where ID=$ID");
	 ...
	 case "2":
	      ...
	      $ID=$_POST['txtId'];
			...
			mysql_query("update otherlink set facebook='$_POST[txtfacebook]',twitter='$_POST[txttwitter]',youtub='$_POST[txtyoutub]' where ID=$ID");
           ...
           echo "  window.location = 'admin_otherlink.php' ";
           echo " </script>";
		     }
			 else
			 {
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_otherlink.php?page=$page' ";
           echo " </script>";
			 }
	 break;
	 case "4":
	   ...
	   mysql_query("Delete from otherlink where ID=$ID");
...
<script language='javascript' type='text/javascript'>
    function btnsave_Click()
	{
		 if(document.getElementById('txtId').value=='-1')
		 {
			 ...
			 {
			    alert(' من فضلك اكتب لينك الفيس بوك.');
			   return false;
			 }
...
<h3 class="hndle"><span>مواقع التواصل الاجتماعى</span></h3>
<div style="" class="inside">
	...
	<form id="frmcategory" action="" method="POST" enctype="multipart/form-data">
      ...
      <input type='hidden' id='txtAction' name='txtAction' value=''>
		...
		<h4 id="quick-post-title"><label for="title">الفيس بوك&nbsp;</label></h4>
		<div >
			<input name="txtfacebook" id="txtfacebook" tabindex="1" size='180' type="text" value="<?=$facebook;?>">
		</div>

		<br/>
		
		<h4 id="quick-post-title"><label for="title">تويتر &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label></h4>
		<div >
			<input name="txttwitter" id="txttwitter" tabindex="1" size='180' type="text" value="<?=$twitter;?>">
		</div>
		
		<br/>
		
		<h4 id="quick-post-title"><label for="title">اليوتيوب &nbsp;&nbsp;&nbsp;&nbsp;</label></h4>
		<div >
			<input name="txtyoutub" id="txtyoutub" tabindex="1" size='180' type="text" value="<?=$youtub;?>">
		</div>
		...
		اضغط على تعديل واكتب
		لينكات مواقع التواصل الاجتماعى
...
$page=$_GET['page'];
	...
	if(!$recordsPerPage){
		...
		$i=($page-1)*$recordsPerPage;
		 ...
		 $date= @date("Y-m-d") ;
		$countRes=mysql_query("select count(ID)from  otherlink  ");
		 ...
		 $date= @date("Y-m-d") ;
		if(!$result=mysql_query("select * from  otherlink   order by ID desc  limit $i,$recordsPerPage  ")){
		...
		if($page>1){
		echo "<td>";		
		echo "<a class='zo' href= admin_otherlink.php>الاولى</a>";
		echo "</td>";
        echo "<td>";		
		echo "<a class='zo' href= admin_otherlink.php?page=".($page-1).">السابق</a>";
		echo "</td>";
	}else{
	...
	//here also i must change the path to the same page that i work in;
	    echo "<td>";
		echo "<a class='zo' href= admin_otherlink.php?page=".($page+1).">التالى</a>";
		echo "</td>";
		echo "<td>";
		echo "<a class='zo' href= admin_otherlink.php?page=".$totalPages.">الاخيره</a>";
		echo "</td>";
	}else{
...
<table class="wp-list-table widefat fixed posts" cellspacing="0">
	<thead>
	<tr>
        <th style='width:50px;' scope="col" id="categories" class="manage-column column-categories" style="">الرقم</th>
        <th style='width:150px;' scope="col" id="categories" class="manage-column column-categories" style="">الفيس بوك</th>
		<th style='width:150px;' scope="col" id="categories" class="manage-column column-categories" style="">تويتر</th>
		<th style='width:150px;' scope="col" id="categories" class="manage-column column-categories" style="">اليوتيوب</th>
