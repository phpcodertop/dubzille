...
switch($_REQUEST['txtAction'])
{
	...
	case "3":
	   $ID=$_POST['txtId'];
	   $desEdit=mysql_query("select * from tips_address where ID=$ID");
	 ...
	 case "2":
	      ...
	      $ID=$_POST['txtId'];
			...
			mysql_query("update tips_address set address='$_POST[txtaddress]'  where ID=$ID");
           ...
           echo "  window.location = 'admin_tips_address.php' ";
           echo " </script>";
		     }
			 else
			 {
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_tips_address.php?page=$page' ";
           echo " </script>";
			 }
	 break;
...
<h3 class="hndle"><span>عنوان النصائح</span></h3>
<div style="" class="inside">
	...
	<form id="frmcategory" action="" method="POST" enctype="multipart/form-data">
      ...
      <input type='hidden' id='txtAction' name='txtAction' value=''>
		...
		<h4 id="quick-post-title"><label for="title">الكلمة</label></h4>
		<div >
...
$page=$_GET['page'];
	...
	if(!$recordsPerPage){
		...
		$i=($page-1)*$recordsPerPage;
		 ...
		 $date= @date("Y-m-d") ;
		$countRes=mysql_query("select count(ID)from tips_address  where ID=$User_ID ");
		 ...
		 $date= @date("Y-m-d") ;
		if(!$result=mysql_query("select * from tips_address  where ID=$User_ID  order by ID desc  limit $i,$recordsPerPage  ")){
		...
		if($page>1){
		echo "<td>";		
		echo "<a class='zo' href= admin_tips_address.php>الاولى</a>";
		echo "</td>";
        echo "<td>";		
		echo "<a class='zo' href= admin_tips_address.php?page=".($page-1).">السابق</a>";
		echo "</td>";
	}else{
	...
	//here also i must change the path to the same page that i work in;
	    echo "<td>";
		echo "<a class='zo' href= admin_tips_address.php?page=".($page+1).">التالى</a>";
		echo "</td>";
		echo "<td>";
		echo "<a class='zo' href= admin_tips_address.php?page=".$totalPages.">الاخيره</a>";
		echo "</td>";
	}else{
...
<table class="wp-list-table widefat fixed posts" cellspacing="0">
	...
	<tr>
        ...
        <th style='width:80%;' scope="col" id="categories" class="manage-column column-categories" style="">الكلمة</th>
