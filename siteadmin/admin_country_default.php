...
switch($_REQUEST['txtAction'])
{
	...
	case "3":
	   $ID=$_POST['txtId'];
	   $desEdit=mysql_query("select * from country_default where ID=$ID");
	   $res=mysql_fetch_array($desEdit);
	   $country_id=$res['country_id'];
	   $country_address=$res['country_address'];
	 ...
	 case "2":
		  ...
		  $decity=mysql_query("select * from country where ID='$_POST[ddlcountry]' ");
	      $rescity=mysql_fetch_array($decity);
		  $country_id=$rescity['ID'];
		  $country_address=$rescity['address'];	
		  
		  
		  mysql_query("update country_default set country_id='$country_id',country_address='$country_address' where ID=$ID");
           ...
           echo "  window.location = 'admin_country_default.php' ";
           echo " </script>";
		     }
			 else
			 {
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_country_default.php?page=$page' ";
           echo " </script>";
			 }
	 break;
...
<h3 class="hndle"><span>الدولة الافتراضية</span></h3>
<div style="" class="inside">
	...
	<form id="frmcategory" action="" method="POST" enctype="multipart/form-data">
      ...
      <input type='hidden' id='txtAction' name='txtAction' value=''>
		...
		<h4 id="quick-post-title"><label for="title">الدولة</label></h4>
		<div >
							<select id='ddlcountry' name='ddlcountry' style="width:100%">
							  
							  <option value='<?=$country_id;?>'><?=$country_address;?></option>
							     <?php
...
$page=$_GET['page'];
	...
	if(!$recordsPerPage){
		...
		$i=($page-1)*$recordsPerPage;
		 ...
		 $date= @date("Y-m-d") ;
		$countRes=mysql_query("select count(ID)from  country_default  ");
		 ...
		 $date= @date("Y-m-d") ;
		if(!$result=mysql_query("select * from  country_default   order by ID desc  limit $i,$recordsPerPage  ")){
		...
		if($page>1){
		echo "<td>";		
		echo "<a class='zo' href= admin_country_default.php>الاولى</a>";
		echo "</td>";
        echo "<td>";		
		echo "<a class='zo' href= admin_country_default.php?page=".($page-1).">السابق</a>";
		echo "</td>";
	}else{
	...
	//here also i must change the path to the same page that i work in;
	    echo "<td>";
		echo "<a class='zo' href= admin_country_default.php?page=".($page+1).">التالى</a>";
		echo "</td>";
		echo "<td>";
		echo "<a class='zo' href= admin_country_default.php?page=".$totalPages.">الاخيره</a>";
		echo "</td>";
	}else{
...
<table class="wp-list-table widefat fixed posts" cellspacing="0">
	...
	<tr>
        ...
        <th style='width:80%;' scope="col" id="categories" class="manage-column column-categories" style="">الدولة</th>
