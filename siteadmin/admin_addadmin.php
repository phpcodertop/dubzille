...
switch($_REQUEST['txtAction'])
{
	case "1":
          ...
          mysql_query(" insert into admin  (admin_name,username,password) values ('$_POST[txtadmin_name]','$_POST[txtadmin_username]','$_POST[txtpassword]')");
           ...
           echo "  window.location = 'admin_addadmin.php' ";
           echo " </script>";
		     }
			 else
			 {
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_addadmin.php?page=$page' ";
           echo " </script>";
			 }
...
<h3 class="hndle"><span>أدمين الموقع</span></h3>
...
$page=$_GET['page'];
	...
	if(!$recordsPerPage){
		...
		if($page>1){
		echo "<td>";		
		echo "<a class='zo' href= admin_addadmin.php>الاولى</a>";
		echo "</td>";
        echo "<td>";		
		echo "<a class='zo' href= admin_addadmin.php?page=".($page-1).">السابق</a>";
		echo "</td>";
	}else{
	...
	//here also i must change the path to the same page that i work in;
	    echo "<td>";
		echo "<a class='zo' href= admin_addadmin.php?page=".($page+1).">التالى</a>";
		echo "</td>";
		echo "<td>";
		echo "<a class='zo' href= admin_addadmin.php?page=".$totalPages.">الاخيره</a>";
		echo "</td>";
	}else{
