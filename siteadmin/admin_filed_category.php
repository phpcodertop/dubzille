...
switch($_REQUEST['txtAction'])
{
	case "1":
		  ...
		  $decategory=mysql_query("select * from  ads_cat where ID='$_POST[ddlads_cat]' ");
	      $rescategory=mysql_fetch_array($decategory);
		  $cata_id=$rescategory['ID'];
		  $cata_address=$rescategory['address'];		  
		  
          mysql_query(" insert into filed_category  (address,cata_id,cata_address) values ('$_POST[txtaddress]','$cata_id','$cata_address')");

		  
           if($page=='')
             {		   
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_filed_category.php' ";
           echo " </script>";
		     }
			 else
			 {
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_filed_category.php?page=$page' ";
           echo " </script>";
			 }
	...
	case "3":
	   $ID=$_POST['txtId'];
	   $desEdit=mysql_query("select * from filed_category where ID=$ID");
	   $res=mysql_fetch_array($desEdit);
	   $address=$res['address'];
	   $cata_id=$res['cata_id'];
	   $cata_address=$res['cata_address'];
	 ...
	 case "2":
		  ...
		  $cata_address=$rescategory['address'];	  
			...
			mysql_query("update filed_category set address='$_POST[txtaddress]',cata_id='$cata_id',cata_address='$cata_address' where ID=$ID");
	 ...
	 case "4":
	   ...
	   mysql_query("Delete from filed_category where ID=$ID");
...
<script language='javascript' type='text/javascript'>
    function btnsave_Click()
	{
		 ...
		 {
			 if(document.getElementById('ddlads_cat').value=='')
			 {
...
<h2>اضافة بيان للاقسام الرئيسية</h2>
...
<div style="" class="inside">
	...
	<form id="frmcategory" action="" method="POST" enctype="multipart/form-data">
      ...
      <input type='hidden' id='txtAction' name='txtAction' value=''>
		...
		<div >
							<select id='ddlads_cat' name='ddlads_cat' style="width:100%">
							  <option value='<?=$cata_id;?>'><?=$cata_address;?></option>
							     <?php
	    ...
	    <h4 id="quick-post-title"><label for="title">البيان</label></h4>
		<div >
...
$cat_id=$_GET['cat_id'];
	...
	if(!$recordsPerPage){
		...
		$i=($page-1)*$recordsPerPage;
		 ...
		 $date= @date("Y-m-d") ;
		$countRes=mysql_query("select count(ID)from filed_category  ");
		 ...
		 $date= @date("Y-m-d") ;
		if(!$result=mysql_query("select * from  filed_category  order by ID desc  limit $i,$recordsPerPage  ")){
		...
		if($page>1){
		echo "<td>";		
		echo "<a class='zo' href= admin_filed_category.php>الاولى</a>";
		echo "</td>";
        echo "<td>";		
		echo "<a class='zo' href= admin_filed_category.php?page=".($page-1).">السابق</a>";
		echo "</td>";
	}else{
	...
	//here also i must change the path to the same page that i work in;
	    echo "<td>";
		echo "<a class='zo' href= admin_filed_category.php?page=".($page+1).">التالى</a>";
		echo "</td>";
		echo "<td>";
		echo "<a class='zo' href= admin_filed_category.php?page=".$totalPages.">الاخيره</a>";
		echo "</td>";
	}else{
...
<table class="wp-list-table widefat fixed posts" cellspacing="0">
	<thead>
	<tr>
        <th style='width:10%;' scope="col" id="categories" class="manage-column column-categories" style="">التسلسل</th>
		<th style='width:30%;' scope="col" id="categories" class="manage-column column-categories" style="">البيان</th>
		<th style='width:40%;' scope="col" id="categories" class="manage-column column-categories" style="">القسم</th>
