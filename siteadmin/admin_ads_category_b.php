...
الاقسام الفرعية للقسم

<?php
include_once("../admin/connection.php");
		$deeteatcher=mysql_query("select * from   ads_cat where ID='$cat_id' ");
	        $resteatcher=mysql_fetch_array($deeteatcher);
	        $cat_id=$resteatcher['ID'];
		$cat_address=$resteatcher['address'];
		echo $cat_address ;
?>	
...
switch($_REQUEST['txtAction'])
{
	case "1":
          ...
          mysql_query(" insert into ads_catb  (address,cat_id,cat_name,appearance) values ('$_POST[txtaddress]','$cat_id','$cat_address','$_POST[ddlappearance]')");
		  
           if($page=='')
             {		   
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_ads_category_b.php?cat_id=$cat_id' ";
           echo " </script>";
		     }
			 else
			 {
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_ads_category_b.php?cat_id=$cat_id&page=$page' ";
           echo " </script>";
			 }
	...
	case "3":
	   $ID=$_POST['txtId'];
	   $desEdit=mysql_query("select * from ads_catb where ID=$ID");
	 ...
	 case "2":
	      ...
	      $ID=$_POST['txtId'];
			...
			mysql_query("update ads_catb set address='$_POST[txtaddress]',appearance='$_POST[ddlappearance]' where ID=$ID");
	 ...
	 case "4":
	   ...
	   mysql_query("Delete from ads_catb where ID=$ID");
...
<script language='javascript' type='text/javascript'>
    function btnsave_Click()
	{
		 if(document.getElementById('txtId').value=='-1')
		 {
			 ...
			 {
			    alert('من فضلك اكتب عنوان القسم.');
			   return false;
			 }
...
<h2>الاقسام الفرعية للاعلانات</h2>
...
<h3 class="hndle"><span>الاقسام الفرعية للقسم
&nbsp;&nbsp;&nbsp;
<?php
		  $deeteatcher=mysql_query("select * from   ads_cat where ID='$cat_id' ");
	      $resteatcher=mysql_fetch_array($deeteatcher);
...
<div style="" class="inside">
	...
	<form id="frmcategory" action="" method="POST" enctype="multipart/form-data">
      ...
      <input type='hidden' id='txtAction' name='txtAction' value=''>
	    ...
	    <h4 id="quick-post-title"><label for="title">عنوان القسم</label></h4>
		<div >
...
$cat_id=$_GET['cat_id'];
	...
	if(!$recordsPerPage){
		...
		$i=($page-1)*$recordsPerPage;
		 ...
		 $date= @date("Y-m-d") ;
		$countRes=mysql_query("select count(ID)from ads_catb where cat_id='$cat_id' ");
		 ...
		 $date= @date("Y-m-d") ;
		if(!$result=mysql_query("select * from ads_catb  where cat_id='$cat_id'  order by ID desc  limit $i,$recordsPerPage  ")){
		...
		if($page>1){
		echo "<td>";		
		echo "<a class='zo' href= admin_ads_category_b.php?cat_id=$cat_id>الاولى</a>";
		echo "</td>";
        echo "<td>";		
		echo "<a class='zo' href= admin_ads_category_b.php?cat_id=$cat_id&page=".($page-1).">السابق</a>";
		echo "</td>";
	}else{
	...
	//here also i must change the path to the same page that i work in;
	    echo "<td>";
		echo "<a class='zo' href= admin_ads_category_b.php?cat_id=$cat_id&page=".($page+1).">التالى</a>";
		echo "</td>";
		echo "<td>";
		echo "<a class='zo' href= admin_ads_category_b.php?cat_id=$cat_id&page=".$totalPages.">الاخيره</a>";
		echo "</td>";
	}else{
...
<table class="wp-list-table widefat fixed posts" cellspacing="0">
	...
	<tr>
        ...
        <th style='width:50%;' scope="col" id="categories" class="manage-column column-categories" style="">عنوان القسم</th>
...
<?php
	...
	&nbsp;&nbsp;&nbsp;
	<a href='admin_ads.php?cat_id=<?=$record['0'];?>'> الاعلانات </a>
	</td>
	</tr>
