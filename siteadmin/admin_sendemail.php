...
switch($_REQUEST['txtAction'])
{
	case "1":
          ...
          mysql_query(" insert into sendmesage  (address,details) values ('$_POST[txtaddress]','$_POST[txtdetails]')");
		  
		 $address_message=$_POST[txtaddress];
		 $details_message=$_POST[txtdetails];
		...
		$other_email=$resemail['other_email'];		 
		 ...
		 $dedc=mysql_query('select * from emiallist  ');
...
<body>
  <center>
  <table class="tt" dir="rtl" width="600">
  <tr><td><font size="5" color="#b80007">'.$sitename.' &nbsp;&nbsp; (رسالة دعائية)</font></td></tr>
  <tr><td><br/><br/></td></tr>
  <tr><td>  '.$address_message.'  </td></tr>
  <tr><td>  '.$address_message.'  </td></tr>
  <tr><td><br/><br/></td></tr>
  ...
  <tr><td><font size="4">لمزيد من المعلومات زوروا موقعنا </font> <a href="'.$link.'"> '.$sitename.' </a></td></tr>
	   ...
	   mail($email_to, $subject, $message, $headers);
           ...
           if($page=='')
             {		   
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_sendemail.php' ";
           echo " </script>";
		     }
			 else
			 {
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_sendemail.php?page=$page' ";
           echo " </script>";
			 }
	...
	break;
	 ...
	 case "4":
	   ...
	   mysql_query("Delete from sendmesage where ID=$ID");
...
<h3 class="hndle"><span>ارسال رسائل</span></h3>
<div style="" class="inside">
	...
	<form id="frmcategory" action="" method="POST" enctype="multipart/form-data">
      ...
      <input type='hidden' id='txtAction' name='txtAction' value=''>
		...
		<h4 id="quick-post-title"><label for="title">عنوان الرسالة</label></h4>
		<div >
		...
		<h4 id="content-label"><label for="content">الرسالة</label></h4>
		<div >
			<textarea name="txtdetails" id="txtdetails" class="mceEditor" rows="30" style="width:100%" tabindex="2"><?=$details;?></textarea>
<script type="text/javascript">
if (document.location.protocol == 'file:') {
	alert("The examples might not work properly on the local file system due to security settings in your browser. Please use a real webserver.");
		...
		<p class="submit">
		        <input name="btnsave" id="btnsave" class="button-primary" value="ارسال" type="submit" onclick="return btnsave_Click()" >
...
$page=$_GET['page'];
	...
	if(!$recordsPerPage){
		...
		$i=($page-1)*$recordsPerPage;
		 ...
		 $date= @date("Y-m-d") ;
		$countRes=mysql_query("select count(ID)from sendmesage  ");
		 ...
		 $date= @date("Y-m-d") ;
		if(!$result=mysql_query("select * from sendmesage   order by ID desc  limit $i,$recordsPerPage  ")){
		...
		if($page>1){
		echo "<td>";		
		echo "<a class='zo' href= admin_sendemail.php>الاولى</a>";
		echo "</td>";
        echo "<td>";		
		echo "<a class='zo' href= admin_sendemail.php?page=".($page-1).">السابق</a>";
		echo "</td>";
	}else{
	...
	//here also i must change the path to the same page that i work in;
	    echo "<td>";
		echo "<a class='zo' href= admin_sendemail.php?page=".($page+1).">التالى</a>";
		echo "</td>";
		echo "<td>";
		echo "<a class='zo' href= admin_sendemail.php?page=".$totalPages.">الاخيره</a>";
		echo "</td>";
	}else{
...
<table class="wp-list-table widefat fixed posts" cellspacing="0">
	...
	<tr>
        ...
        <th style='width:20%;' scope="col" id="categories" class="manage-column column-categories" style="">عنوان الرسالة</th>
		<th style='width:60%;' scope="col" id="categories" class="manage-column column-categories" style="">الرسالة</th>
