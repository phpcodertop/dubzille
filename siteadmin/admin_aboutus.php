...
switch($_REQUEST['txtAction'])
{
	...
	case "3":
	   $ID=$_POST['txtId'];
	   $desEdit=mysql_query("select * from aboutus where ID=$ID");
	 ...
	 case "2":
	      ...
	      $ID=$_POST['txtId'];
			...
			mysql_query("update aboutus set details='$_POST[txtdetails]' where ID=$ID");
           ...
           echo "  window.location = 'admin_aboutus.php' ";
           echo " </script>";
		     }
			 else
			 {
		   echo "<script type=text/javascript>";
           echo "  window.location = 'admin_aboutus.php?page=$page' ";
           echo " </script>";
			 }
	 break;
	}	
...
<h3 class="hndle"><span>من نحن</span></h3>
...
$page=$_GET['page'];
	...
	if(!$recordsPerPage){
		...
		$i=($page-1)*$recordsPerPage;
		 ...
		 $date= @date("Y-m-d") ;
		$countRes=mysql_query("select count(ID)from aboutus  ");
		 ...
		 $date= @date("Y-m-d") ;
		if(!$result=mysql_query("select * from aboutus   order by ID desc  limit $i,$recordsPerPage  ")){
		...
		if($page>1){
		echo "<td>";		
		echo "<a class='zo' href= admin_aboutus.php>الاولى</a>";
		echo "</td>";
        echo "<td>";		
		echo "<a class='zo' href= admin_aboutus.php?page=".($page-1).">السابق</a>";
		echo "</td>";
	}else{
	...
	//here also i must change the path to the same page that i work in;
	    echo "<td>";
		echo "<a class='zo' href= admin_aboutus.php?page=".($page+1).">التالى</a>";
		echo "</td>";
		echo "<td>";
		echo "<a class='zo' href= admin_aboutus.php?page=".$totalPages.">الاخيره</a>";
		echo "</td>";
	}else{
...
<table class="wp-list-table widefat fixed posts" cellspacing="0">
	...
	<tr>
        ...
        <th style='width:80%;' scope="col" id="categories" class="manage-column column-categories" style="">من نحن</th>
